...
def create_data_lists(voc07_path, voc12_path, output_folder):
    ...
    voc07_path_test = os.path.abspath(".\\media\\ssd\\ssd data\\VOC2007\\")
    ...
    with open(os.path.join(voc07_path_test, 'ImageSets\\Main\\test.txt')) as f:
        ids = f.read().splitlines()
...
def Load_data_file(path):
    ...
    train_dataset = datasets.CIFAR10(path, train=True, transform=transforms.ToTensor(), download=True)
    train_loader = DataLoader(train_dataset, batch_size=64, shuffle=True)
    test_dataset = datasets.CIFAR10(path, train=False, transform=transforms.ToTensor(), download=True)
