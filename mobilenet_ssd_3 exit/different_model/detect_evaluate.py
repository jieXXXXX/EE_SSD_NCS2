...
model_backbone0_xml = "./weight/weight_FP32/backbone1.xml"
model_backbone0_bin = "./weight/weight_FP32/backbone1.bin" 
model_backbone1_xml = "./weight/weight_FP32/backbone2.xml"
model_backbone1_bin = "./weight/weight_FP32/backbone2.bin" 
model_backbone2_xml = "./weight/weight_FP32/backbone3.xml"
model_backbone2_bin = "./weight/weight_FP32/backbone3.bin" 
...
#-----------------------------exit strategy-------------------------------
def exit_criterion_top(prediction,location,exit_threshold): #NOT for batch size > 1
...
def last_process(det_boxes,det_labels,det_scores,exit_point):
    ...
    text_size = font.getbbox(exit_point[i].upper())
    textbox_exit_location = [10, 20 - text_size[1], 
                        10 + text_size[0] + 4., 20] 
    draw.text(xy=textbox_exit_location, text=exit_point.upper(), fill='white',
...
def inference_main(images):
    ...
    inference_time.append(infer_sub1_time+infer_backbone0_time)
    
    #------------------------------decide if need to early exit---------------
    exit_bool = exit_criterion_entropy(result['output1'],result['output2'],exit_threshold1)
    exit_point = 0
    ...
    if(not exit_bool):
        ...
        exit2_bool = exit_criterion_entropy(result['output1'],result['output2'],exit_threshold2)
        exit_point = 1
        if(not exit2_bool):
            exit_point = 2
    ...
    return result,exit_point,inference_time,device_name
...
def evaluate()   :
    ...
    eixt_1_inference_time = AverageMeter()  # loss
    ...
    with torch.no_grad():
        ...
        for i, (images, boxes, labels, difficulties) in enumerate(test_loader):
            ...
            result,exit_point,inference_time,device_name= inference_main(images)
            ...
            if i>test_image_number:break
        
          
        #---------------output the result------------------------
        print("----------------------------------------------")        
        print("device name is ",device_name)
        print("loss function is exit_criterion_entropy", "with exit_threshold1:",exit_threshold1,"and exit_threshold2:",exit_threshold2,)
        ...
        print("probability Final early exit:" ,exit_F_num/i*100,"%",",with",eixt_Final_inference_time.avg,"for each image")
        print()
