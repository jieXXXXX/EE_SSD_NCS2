...
warnings.filterwarnings("ignore", category=UserWarning)#屏蔽掉一些不重要的warning信息
...
IMAGE_PATH = "./test_detect_image/008910.jpg"
...
def last_process(det_boxes,det_labels,det_scores,exit_point):
    ...
    #width = int(sqrt(np.log(original_image.size[0])))
    width = int(0.3*sqrt(sqrt(original_image.size[0])))
    font = ImageFont.truetype("./calibril.ttf", 30*width)
    ...
    textbox_exit_location = [10, 20 - text_size[1], 
                        10 + text_size[0] + 4., 20] 
    draw.text(xy=textbox_exit_location, text=exit_point.upper()+" exit point /VGG16", fill='white',
