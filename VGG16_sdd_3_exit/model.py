...
class SSD300(nn.Module):
    ...
    def __init__(self, n_classes,exit_threshold=0.5):  
        ...
        self.fast_inference_mode = False    # if fast_inference_mode==True ,use early exit model, while transver the whold backbone
        self.exit_threshold = torch.tensor([exit_threshold], dtype=torch.float32)#set the exit threshold ,while when using NCS2 it don't need to consider this parameter
        self.exit_loss_weights = [1.0,1.0,1.0] #weighting for each exit when summing loss,original value is[1.0,0.3]
        self.n_classes = n_classes
        
        #create the backbone and the sub network
    ...
    def _forward_training(self, x ,train_bb):
        ...
        if train_bb:
            ...
            conv4_3_feats,conv7_feats = (self.backbone)[2](conv4_3_feats,x)
            
            loc, classes_score = self.exits[-1](conv4_3_feats,conv7_feats)
            return loc, classes_score
        ...
        #---------------------------train the subnetwork----------------------
            ...
            for bb, ee in zip(self.backbone, self.exits):
                ...
                if "VGGBase_backbone1" in str(bb):
                    #backbone[first] and exit[first]
                    x = bb(x)                 
                    loc, classes_score = ee(x)
                
                if "VGGBase_backbone2" in str(bb):    
                    #backbone[second] and exit[second]
                    conv4_3_feats, x = bb(x)
                    loc, classes_score = ee(conv4_3_feats,x)
                    
                if "VGGBase_backbone3" in str(bb):
                    #backbone[final] and exit[final]
        ...
        if self.fast_inference_mode:
            ...
            for bb, ee in zip(self.backbone, self.exits):
                ...
                if "VGGBase_backbone3" in str(bb):
                    ...
                    exit_before = True
                    exit_point = "final exit point"

                
                if(not exit_before):
                    if self.exit_criterion_top1(classes_score,loc):
                        print("exit at", exit_point)
                        return loc, classes_score
            ...
            #training
            return self._forward_training(x,train_bb)
    ...
    def exit_criterion_top1(self, prediction,location): #NOT for batch size > 1
        ...
        with torch.no_grad():
            exit_bool=False
            prediction = F.softmax(prediction, dim=2)
            max_prediction=np.max(np.array((prediction[0][:,1:-1]).cpu()))#detele background class
    ...
    def load_pretrained_layers(self):
       ...
       # Current state of base
        ...
        for i, param in enumerate(param_names[:26]):  # excluding conv6 and conv7 parameters
            #ignore ' backbone.1.rescale_factors' layer
            if(i<14):
        ...
        state_dict['backbone.2.conv6.weight'] = decimate(conv_fc6_weight, m=[4, None, 3, 3])  # (1024, 512, 3, 3)
        state_dict['backbone.2.conv6.bias'] = decimate(conv_fc6_bias, m=[4])  # (1024)
        # fc7
        conv_fc7_weight = pretrained_state_dict['classifier.3.weight'].view(4096, 4096, 1, 1)  # (4096, 4096, 1, 1)
        conv_fc7_bias = pretrained_state_dict['classifier.3.bias']  # (4096)
        state_dict['backbone.2.conv7.weight'] = decimate(conv_fc7_weight, m=[4, 4, None, None])  # (1024, 1024, 1, 1)
        state_dict['backbone.2.conv7.bias'] = decimate(conv_fc7_bias, m=[4])  # (1024)
